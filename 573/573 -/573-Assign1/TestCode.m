...
aTemp(FlowOutliers,i) = TempMean(1,i);
...
bTemp(FlowOutliers,i) = TempMean(3,i);
...
aEnergyBalance(:,1) = (aFlowRates(:, 1).*aTemp(:,1) + aFlowRates(:, 2).*aTemp(:,2))./(aFlowRates(:, 1) + aFlowRates(:, 2));
aEnergyBalance(:,2) = aTemp(:,3);
aEnergyBalance(:,4) = aTemp(:,4);

bEnergyBalance(:,1) = (bFlowRates(:, 1).*bTemp(:,1) + bFlowRates(:, 2).*bTemp(:,2))./(bFlowRates(:, 1) + bFlowRates(:, 2));
